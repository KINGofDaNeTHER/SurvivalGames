package me.kingofdanether.survivalgames.configuration;
...
public class ConfigFileManager 
{
	...
	public ConfigFile getNewConfigFromJar(String fileName) {
		File file = new File(plugin.getDataFolder() + File.separator + fileName);
		
		if(!file.exists())
		{
			try {
				SurvivalGames.getInstance().saveResource(fileName, false);
			} catch (Exception e) {
				e.printStackTrace();
				SurvivalGames.getInstance().getLogger().warning("§4" + fileName + " not found. Disabling...");
				SurvivalGames.getInstance().disable();
				return null;
			}
			this.prepareFile(fileName);
		}
