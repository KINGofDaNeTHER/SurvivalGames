...
public class RemoveArena implements CommandExecutor {
	...
	private static RemoveArena instance;
	
	public static RemoveArena getInstance() {
		if (instance == null) {
			return new RemoveArena();
		}
		return instance;
	}
	...
	public boolean onCommand(CommandSender sender, Command cmd, String label, String[] args) {
		if (cmd.getName().equalsIgnoreCase("sg")) {
			if (!sender.hasPermission("survivalgames.removearena")) {
			...
			if (a == null) {
				...
				a.getDataFile().delete();
				ArenaManager.removeArena(a);
				sender.sendMessage(StringUtils.colorize(Constants.PREFIX + " &aDeleted arena called \"" + a.getName() + "\"!"));
			}
		}
		return true;
	}
